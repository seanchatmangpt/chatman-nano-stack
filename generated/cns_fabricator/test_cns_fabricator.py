...
def test_8tick_compliance():
    ...
    cmd = ["./cns_fabricator_test"]
    result = subprocess.run(cmd, capture_output=True, text=True)
    
    if result.returncode == 0:
        print(f"✅ cns_fabricator passes 8-tick compliance")
        return True
    else:
        print(f"❌ cns_fabricator fails 8-tick compliance")
        print(result.stdout)
        return False
...
def stress_test(iterations=10000):
    ...
    def run_test():
        ...
        subprocess.run(["./cns_fabricator_test"], capture_output=True)
...
def adversarial_test():
    ...
    for inp in malicious_inputs:
        proc = subprocess.Popen(["./cns_fabricator_test"], 
...
if __name__ == "__main__":
    print(f"🧪 Testing cns_fabricator...")
