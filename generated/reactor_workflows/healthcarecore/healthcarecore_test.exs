...
defmodule HealthcareCore.ValidationTest do
  @moduledoc """
  Comprehensive validation tests for HealthcareCore Reactor workflow
  """
  
  use ExUnit.Case, async: true
  
  describe "HealthcareCore workflow validation" do
    test "processes all semantic concepts correctly" do
      ...
      {:ok, result} = Reactor.run(HealthcareCore.Workflow, input_data)
    ...
    test "meets performance requirements" do
      ...
      {time_microseconds, {:ok, _result}} = :timer.tc(fn ->
        Reactor.run(HealthcareCore.Workflow, input_data)
      end)
    ...
    test "handles BitActor integration failures gracefully" do
      ...
      case Reactor.run(HealthcareCore.Workflow, input_data) do
        {:error, _reason} -> 
    ...
    test "saga compensation works correctly" do
      ...
      {:error, _reason} = Reactor.run(HealthcareCore.Workflow, input_data)
