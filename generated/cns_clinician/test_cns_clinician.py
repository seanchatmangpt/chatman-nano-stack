...
def test_8tick_compliance():
    ...
    cmd = ["./cns_clinician_test"]
    result = subprocess.run(cmd, capture_output=True, text=True)
    
    if result.returncode == 0:
        print(f"✅ cns_clinician passes 8-tick compliance")
        return True
    else:
        print(f"❌ cns_clinician fails 8-tick compliance")
        print(result.stdout)
        return False
...
def stress_test(iterations=10000):
    ...
    def run_test():
        ...
        subprocess.run(["./cns_clinician_test"], capture_output=True)
...
def adversarial_test():
    ...
    for inp in malicious_inputs:
        proc = subprocess.Popen(["./cns_clinician_test"], 
