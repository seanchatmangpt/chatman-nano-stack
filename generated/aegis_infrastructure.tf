...
# Generated from TTL: 2025-07-24T20:24:31.000599
...
resource "kubernetes_namespace" "aegis_fabric" {
  metadata {
    ...
    labels = {
      "app"                        = "aegis-fabric"
      "version"                    = "2025-07-24-20-24-31.000599"
...
resource "kubernetes_secret" "aegis_encryption_keys" {
  ...
  data = {
    gossip_key = base64encode("aegis-gossip-${substr(sha256("2025-07-24T20:24:31.000599"), 0, 16)}")
    ...
    "aegis.rules" = <<-EOT
      ...
      - name: aegis.rules
        ...
        - alert: AegisThreatDetectionRateLow
          expr: aegis_threat_detection_rate < 0.99
          for: 5m
          labels:
            severity: critical
            component: aegis
          annotations:
            ...
            description: "Detection rate {{ $value }} is below target 0.99"
        
        - alert: AegisFalsePositiveRateHigh
          expr: aegis_false_positive_rate > 0.01
          for: 5m
          labels:
            severity: warning
            component: aegis
          annotations:
            ...
            description: "False positive rate {{ $value }} exceeds threshold 0.01"
        
        - alert: AegisGossipConvergenceSlow
          expr: aegis_gossip_convergence_ms > 1000
          for: 2m
          labels:
            severity: warning
            component: gossip
          annotations:
            ...
            description: "Convergence time {{ $value }}ms exceeds target 1000ms"
