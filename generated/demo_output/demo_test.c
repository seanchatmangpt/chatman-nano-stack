/**
 * BitActor Unit Tests - Generated from bitactor_semantic_core
 * Test 8-tick performance guarantee and signal processing
 * Generated by CNS Jinja AOT Compiler
 */

#include <stdio.h>
#include <stdlib.h>
#include <assert.h>
#include <time.h>
#include <string.h>

#define DEMO_IMPLEMENTATION
#include "demo_bitactor.h"

/* Test utilities */
static uint64_t get_timestamp_ns() {
    struct timespec ts;
    clock_gettime(CLOCK_MONOTONIC, &ts);
    return ts.tv_sec * 1000000000ULL + ts.tv_nsec;
}

/* Test cases */
static void test_demo_init() {
    printf("Testing demo_bitactor_init...\n");
    
    demo_bitactor_t ba;
    demo_bitactor_init(&ba);
    
    assert(ba.signal_head == 0);
    assert(ba.signal_tail == 0);
    assert(ba.tick_count == 0);
    assert(ba.signal_count == 0);
    
    assert(ba.dispatch[DEMO_SIGNAL_SEMANTICSIGNAL] == demo_handle_semanticsignal_handler);
    assert(ba.dispatch[DEMO_SIGNAL_NE447DE74ABCE48AC9916FED1B319BD2BB6] == demo_handle_ne447de74abce48ac9916fed1b319bd2bb6_handler);
    assert(ba.dispatch[DEMO_SIGNAL_NE447DE74ABCE48AC9916FED1B319BD2BB7] == demo_handle_ne447de74abce48ac9916fed1b319bd2bb7_handler);
    assert(ba.dispatch[DEMO_SIGNAL_HEARTBEATSIGNAL] == demo_handle_heartbeatsignal_handler);
    assert(ba.dispatch[DEMO_SIGNAL_NORMALSIGNAL] == demo_handle_normalsignal_handler);
    assert(ba.dispatch[DEMO_SIGNAL_DEBUGSIGNAL] == demo_handle_debugsignal_handler);
    
    printf("✅ demo_bitactor_init passed\n");
}

static void test_signal_enqueue() {
    printf("Testing signal enqueue...\n");
    
    demo_bitactor_t ba;
    demo_bitactor_init(&ba);
    
    demo_signal_t sig = {
        .type = DEMO_SIGNAL_SEMANTICSIGNAL,
        .flags = 0,
        .timestamp = get_timestamp_ns(),
        .payload = 0xDEADBEEF
    };
    
    bool result = demo_bitactor_enqueue_signal(&ba, &sig);
    assert(result == true);
    assert(ba.signal_head == 1);
    
    printf("✅ Signal enqueue passed\n");
}

static void test_tick_performance() {
    printf("Testing tick performance (8-tick budget)...\n");
    
    demo_bitactor_t ba;
    demo_bitactor_init(&ba);
    
    /* Enqueue test signals */
    demo_signal_t sig1 = {
        .type = DEMO_SIGNAL_SEMANTICSIGNAL,
        .flags = 0,
        .timestamp = get_timestamp_ns(),
        .payload = 1
    };
    demo_bitactor_enqueue_signal(&ba, &sig1);
    demo_signal_t sig2 = {
        .type = DEMO_SIGNAL_NE447DE74ABCE48AC9916FED1B319BD2BB6,
        .flags = 0,
        .timestamp = get_timestamp_ns(),
        .payload = 2
    };
    demo_bitactor_enqueue_signal(&ba, &sig2);
    demo_signal_t sig3 = {
        .type = DEMO_SIGNAL_NE447DE74ABCE48AC9916FED1B319BD2BB7,
        .flags = 0,
        .timestamp = get_timestamp_ns(),
        .payload = 3
    };
    demo_bitactor_enqueue_signal(&ba, &sig3);
    
    /* Process signals and measure performance */
    uint64_t total_ticks = 0;
    int processed = 0;
    
    while (ba.signal_head != ba.signal_tail) {
        uint64_t start = rdtsc();
        demo_bitactor_tick(&ba);
        uint64_t elapsed = rdtsc() - start;
        
        total_ticks += elapsed;
        processed++;
        
        /* Verify tick budget */
        assert(elapsed <= DEMO_TICK_BUDGET);
    }
    
    printf("✅ Processed %d signals, avg ticks: %llu\n", processed, (unsigned long long)(total_ticks / processed));
}

static void test_ring_buffer_overflow() {
    printf("Testing ring buffer overflow protection...\n");
    
    demo_bitactor_t ba;
    demo_bitactor_init(&ba);
    
    demo_signal_t sig = {
        .type = DEMO_SIGNAL_SEMANTICSIGNAL,
        .flags = 0,
        .timestamp = 0,
        .payload = 0
    };
    
    /* Fill the ring buffer */
    int enqueued = 0;
    while (demo_bitactor_enqueue_signal(&ba, &sig)) {
        enqueued++;
        if (enqueued > DEMO_RING_SIZE) {
            break; /* Safety check */
        }
    }
    
    assert(enqueued == DEMO_RING_SIZE - 1);
    
    /* Verify overflow protection */
    bool result = demo_bitactor_enqueue_signal(&ba, &sig);
    assert(result == false);
    
    printf("✅ Ring buffer overflow protection working (capacity: %d)\n", enqueued);
}

static void test_semanticsignal_signal() {
    printf("Testing Semantic Signal signal...\n");
    
    demo_bitactor_t ba;
    demo_bitactor_init(&ba);
    
    demo_signal_t sig = {
        .type = DEMO_SIGNAL_SEMANTICSIGNAL,
        .flags = 0x1234,
        .timestamp = get_timestamp_ns(),
        .payload = 0xCAFEBABE
    };
    
    demo_bitactor_enqueue_signal(&ba, &sig);
    demo_bitactor_tick(&ba);
    
    assert(ba.signal_count == 1);
    printf("✅ Semantic Signal signal processed\n");
}
static void test_ne447de74abce48ac9916fed1b319bd2bb6_signal() {
    printf("Testing ne447de74abce48ac9916fed1b319bd2bb6 signal...\n");
    
    demo_bitactor_t ba;
    demo_bitactor_init(&ba);
    
    demo_signal_t sig = {
        .type = DEMO_SIGNAL_NE447DE74ABCE48AC9916FED1B319BD2BB6,
        .flags = 0x1234,
        .timestamp = get_timestamp_ns(),
        .payload = 0xCAFEBABE
    };
    
    demo_bitactor_enqueue_signal(&ba, &sig);
    demo_bitactor_tick(&ba);
    
    assert(ba.signal_count == 1);
    printf("✅ ne447de74abce48ac9916fed1b319bd2bb6 signal processed\n");
}
static void test_ne447de74abce48ac9916fed1b319bd2bb7_signal() {
    printf("Testing ne447de74abce48ac9916fed1b319bd2bb7 signal...\n");
    
    demo_bitactor_t ba;
    demo_bitactor_init(&ba);
    
    demo_signal_t sig = {
        .type = DEMO_SIGNAL_NE447DE74ABCE48AC9916FED1B319BD2BB7,
        .flags = 0x1234,
        .timestamp = get_timestamp_ns(),
        .payload = 0xCAFEBABE
    };
    
    demo_bitactor_enqueue_signal(&ba, &sig);
    demo_bitactor_tick(&ba);
    
    assert(ba.signal_count == 1);
    printf("✅ ne447de74abce48ac9916fed1b319bd2bb7 signal processed\n");
}
static void test_heartbeatsignal_signal() {
    printf("Testing HeartbeatSignal signal...\n");
    
    demo_bitactor_t ba;
    demo_bitactor_init(&ba);
    
    demo_signal_t sig = {
        .type = DEMO_SIGNAL_HEARTBEATSIGNAL,
        .flags = 0x1234,
        .timestamp = get_timestamp_ns(),
        .payload = 0xCAFEBABE
    };
    
    demo_bitactor_enqueue_signal(&ba, &sig);
    demo_bitactor_tick(&ba);
    
    assert(ba.signal_count == 1);
    printf("✅ HeartbeatSignal signal processed\n");
}
static void test_normalsignal_signal() {
    printf("Testing NormalSignal signal...\n");
    
    demo_bitactor_t ba;
    demo_bitactor_init(&ba);
    
    demo_signal_t sig = {
        .type = DEMO_SIGNAL_NORMALSIGNAL,
        .flags = 0x1234,
        .timestamp = get_timestamp_ns(),
        .payload = 0xCAFEBABE
    };
    
    demo_bitactor_enqueue_signal(&ba, &sig);
    demo_bitactor_tick(&ba);
    
    assert(ba.signal_count == 1);
    printf("✅ NormalSignal signal processed\n");
}
static void test_debugsignal_signal() {
    printf("Testing DebugSignal signal...\n");
    
    demo_bitactor_t ba;
    demo_bitactor_init(&ba);
    
    demo_signal_t sig = {
        .type = DEMO_SIGNAL_DEBUGSIGNAL,
        .flags = 0x1234,
        .timestamp = get_timestamp_ns(),
        .payload = 0xCAFEBABE
    };
    
    demo_bitactor_enqueue_signal(&ba, &sig);
    demo_bitactor_tick(&ba);
    
    assert(ba.signal_count == 1);
    printf("✅ DebugSignal signal processed\n");
}

/* Benchmark suite */
static void benchmark_throughput() {
    printf("\n=== Throughput Benchmark ===\n");
    
    demo_bitactor_t ba;
    demo_bitactor_init(&ba);
    
    const int NUM_SIGNALS = 1000000;
    uint64_t start_time = get_timestamp_ns();
    
    for (int i = 0; i < NUM_SIGNALS; i++) {
        demo_signal_t sig = {
            .type = (i % 6) + 1,
            .flags = 0,
            .timestamp = i,
            .payload = i
        };
        
        while (!demo_bitactor_enqueue_signal(&ba, &sig)) {
            demo_bitactor_tick(&ba);
        }
        
        if (ba.signal_head == ba.signal_tail + 1) {
            demo_bitactor_tick(&ba);
        }
    }
    
    /* Process remaining signals */
    while (ba.signal_head != ba.signal_tail) {
        demo_bitactor_tick(&ba);
    }
    
    uint64_t elapsed_ns = get_timestamp_ns() - start_time;
    double throughput = (double)NUM_SIGNALS / (elapsed_ns / 1e9);
    
    printf("Processed %d signals in %.3f ms\n", NUM_SIGNALS, elapsed_ns / 1e6);
    printf("Throughput: %.2f signals/sec (%.2f MHz)\n", throughput, throughput / 1e6);
    printf("Average latency: %.2f ns/signal\n", (double)elapsed_ns / NUM_SIGNALS);
}

int main(int argc, char** argv) {
    printf("DEMO BitActor Test Suite\n");
    printf("================================\n\n");
    
    /* Run unit tests */
    test_demo_init();
    test_signal_enqueue();
    test_tick_performance();
    test_ring_buffer_overflow();
    
    test_semanticsignal_signal();
    test_ne447de74abce48ac9916fed1b319bd2bb6_signal();
    test_ne447de74abce48ac9916fed1b319bd2bb7_signal();
    test_heartbeatsignal_signal();
    test_normalsignal_signal();
    test_debugsignal_signal();
    
    /* Run benchmarks */
    if (argc > 1 && strcmp(argv[1], "--bench") == 0) {
        benchmark_throughput();
    }
    
    printf("\n✅ All tests passed!\n");
    return 0;
}